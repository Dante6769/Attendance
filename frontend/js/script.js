...
// --------------------------
const BASE_URL = "http://127.0.0.1:5000"; // Change to server IP/domain when deployed
...
function startAttendance() {
    ...
    .then(data=>{
        if(data.status==="session_started"){
            ...
            document.getElementById("qr").src = `${BASE_URL}/generate_qr`;
...
function stopAttendance() {
    fetch(`${BASE_URL}/stop_session`, { method: "POST" });
...
function loadAttendance(){
    ...
    fetch(`${BASE_URL}/attendance_by_division?division=${division}`)
    .then(res => res.json())
    .then(data => {
